...
if [ "$#" -ne 3 ]; then
    echo "usage: ./bench_root.sh ModelName Core Push_model(0/1)"
    exit
fi
...
Core=$2
echo Use CPU core $Core
PUSH_MODEL=$3
BatchSize=(1 2 4 8 16 32)
echo BatchSize: ${BatchSize[*]}
OUT_FILE=train_bench
DEVICE=MEIZU_f0 # replace it with your device name
...
# some devices use "su -c" to get root permission. If "su -0" failed, try "su -c".
function start_monitor() {
    adb push usage_monitor.sh $ANDROID_DIR
    adb shell chmod 0777 $ANDROID_DIR/usage_monitor.sh
    adb shell "su -0 $ANDROID_DIR/usage_monitor.sh $ANDROID_DIR/usage_monitor.result 1"
}

function end_monitor() {
    echo "pid=\`pgrep -f usage\` ; su -0 kill -9 \$pid" > ../cmd.txt 
    adb shell < ../cmd.txt
...
function bench_session() {
    ...
    adb shell "su -0 mv -f $ANDROID_DIR/temp* $ANDROID_DIR/models"
...
function bench_android() {
    echo start monitoring device status
    start_monitor &
    build_android_bench
