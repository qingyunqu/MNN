...
function build_android_bench() {
    ...
    if [ $PUSH_MODEL -ne 0 ]; then
    DEVICE=RedmiNote9P_big_${PUSH_MODEL}
    fi
...
function bench_android() {
    ...
    do
        ...
        if [ $parameter == $ModelName ]; then
            ...
            adb shell "LD_LIBRARY_PATH=$ANDROID_DIR  taskset -a c0 $ANDROID_DIR/build/runTrainDemo.out MnistBenchmark $ANDROID_DIR/mnist_data $parameter $ModelName 2>$ANDROID_DIR/benchmark.err >> $ANDROID_DIR/$OUT_FILE.result"
            echo end
            fi
            # 30
