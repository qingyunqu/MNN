...
echo $core
freq="l"
DEVICE=MEIZU_${core}_${freq}
echo $DEVICE


function change_freq() {
    adb shell "su -0 echo 'userspace' > /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor"
    adb shell "su -0 echo '702000' > /sys/devices/system/cpu/cpu0/cpufreq/scaling_setspeed"
}
...
function end_monitor() {
    echo "pid=\`pgrep -f usage\` ; su -0 kill -9 \$pid" > ../cmd.txt # adb shell中连续执行多条指令的办法
    adb shell < ../cmd.txt 
...
function bench_android() {
    ...
    do
        ...
        if [ $parameter == $ModelName ]; then
            ...
            adb shell "LD_LIBRARY_PATH=$ANDROID_DIR taskset -a $core $ANDROID_DIR/build/runTrainDemo.out MnistBenchmark $ANDROID_DIR/mnist_data $parameter $ModelName 2>$ANDROID_DIR/benchmark.err >> $ANDROID_DIR/$OUT_FILE.result"
            echo end
            fi
            # 30
